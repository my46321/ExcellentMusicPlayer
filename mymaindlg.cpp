...
ui->pushButton_weather->installEventFilter(this);
ui->pushButton_weather->setMouseTracking(true);

ui->widget_menu->setWindowOpacity(0.8);
//this->setWindowOpacity(0.9);
//pushButton_menu->setStyleSheet("QPushButton{border-image: url(:/image/listen1.jpg);}");
//pushButton_menu->setGeometry(QRect(20,30,50,50));
//pushButton_menu->setToolTip("主页");
    ...
    mylovetable=new mymaintablewidget(stack);



    addtips=new QLabel("点击按钮添加\n或拖动文件到界面", mymaintable);//播放列表
    addtips->setGeometry(300,280,180,50);
    ...
    for(int i=0;i<6;i++)
    {
         mylistWidgetItem[i]=ui->listWidget_side->item(i);
    }
    for(int i=0;i<100;i++)
    {
        mylove[i]=0;
    ...
    for(int i=0;i<100;i++)
    {
        myloveptr[i]=new QTableWidgetItem();
    }
    ...
    lrcshowedit->setFont(QFont("微软雅黑", 10, QFont::Bold));
    ...
    //////////////////////////////////////////////////////
        //实例QPushButonn控件
      pushButton_listen= new QPushButton(this);
        button = new QPushButton(this);
        pushButton_menu=new QPushButton(this);
        //按钮显示位置
        button->setGeometry(QRect(0,500,25,25));
        //按钮值
        button->setStyleSheet("QPushButton{border-image: url(:/image/luyin.jpg);}");
         //点击事件
        connect(button,SIGNAL(clicked()),this,SLOT(showrecord()));
        pushButton_listen->setToolTip("随意听");
        pushButton_listen->setGeometry(QRect(510,190,100,100));
      //按钮值
      pushButton_listen->setStyleSheet("QPushButton{border-image: url(:/image/listen1.jpg);}");
    pushButton_menu->setStyleSheet("QPushButton{border-image: url(:/image/listen1.jpg);}");
      pushButton_menu->setGeometry(QRect(20,30,50,50));
      pushButton_menu->setToolTip("主页");
      connect(pushButton_listen,SIGNAL(clicked()),this,SLOT(pushButton_listen_clicked()));
      button->setToolTip("录音");
      button->setGeometry(510,390,100,100);
//              pushButton_menu->setStyleSheet("QPushButton{border-image: url(:/image/listen1.jpg);}");
//    pushButton_menu->setGeometry(QRect(20,30,50,50));
...
/////////////////////////////////////////////////////////////
    ...
    movie->start();
    loadlab->raise();
    loadlab->setGeometry(160,80,64,64);
    loadlab->hide();
    ...
    stack->addWidget(mylovetable);
    stack->setGeometry(160,100,780,600);//播放列表大小
    stack->lower();

    ui->widget_search->raise();//位于最上层
   ui->widget_search->setWindowFlags(Qt::Widget | Qt::FramelessWindowHint | Qt::WindowSystemMenuHint | Qt::WindowStaysOnTopHint);
...
///////////////////////////////////////////////////
    ...
    connect(pushButton_menu,SIGNAL(clicked()),this,SLOT( on_pushButton_menu_clicked()));
    ...
    connect(mylovetable,SIGNAL(itemDoubleClicked(QTableWidgetItem*)),this,SLOT(mylovedoubleclick(QTableWidgetItem *)));
...
void MyMainDlg::changplaybtbsty(QMediaPlayer::State states)
{
    if(states==QMediaPlayer::PlayingState)
    {
        ui->pushButton_playpause->setStyleSheet(pause_style);
        ui->pushButton_playpause->setToolTip("暂停");
        play_button->setStyleSheet("QPushButton{border-image: url(:/image/menupause.png);}");
    }
        if(states==QMediaPlayer::PausedState)
        {
            ////////////////////
            ui->pushButton_playpause->setStyleSheet(play_style);
            ui->pushButton_playpause->setToolTip("播放");
...
void MyMainDlg::shownowplaylabel(QMediaContent media)
{
    ...
    if(nowplaylist==1)
    {
        ...
        QString url4="D://ExcellentAlbum/"+name+".jpg";
        QFile file4(url4);
        if(file4.exists())//如果存在专辑图片 加载图片
         ...
         {
             QString url="D://ExcellentAlbum/"+name+".jpg";
 ...
 /////////////////////////////////////////////////////////////////////////请求专辑图片
      QString url="D://ExcellentAlbum/"+netnowplaysongname+".jpg";
      QFile file(url);
  ...
  ///////////////////////////////////////////////////////////////////////////////请求歌词
       QString url2="D://ExcellentLrc/"+netnowplaysongname+".lrc";
       QFile file2(url2);
...
void MyMainDlg::searchdownui()//下载的界面
{
    ...
    tabwidget->setStyleSheet("background-color: rgb(250,250,250);");
    ...
    downloadtable->setGeometry(0,30,194,442);
    ///////////////////////////////////////
    ...
    frame->setGeometry(5,28,183,1);
    frame->setFrameShadow(QFrame::Raised);
    frame->setFrameShape(QFrame::HLine);

    downstart->setGeometry(5,5,55,20);
    downpause->setGeometry(65,5,55,20);
    downdelall->setGeometry(125,5,55,20);
...
void MyMainDlg::searchsongui()//搜索网络歌曲界面的ui
{
   ///////////////////////////////////////////////////////////////////歌曲搜索框
    ...
    netseawidget->setGeometry(780,132,194,16);//设置动态起始的位置
  ...
  /////////////////////////////////////////////////////////////////////歌曲搜索显示界面
    ...
    searchtable->setFixedSize(194,419);
    vlyout2->setSpacing(0);
    vlyout2->setContentsMargins(0,25,0,0);
...
void MyMainDlg::songanaly()//网络搜索按钮
{
   ...
   song_trans=ledit->text();
   if(song_trans.isEmpty())
       return;
   d=new downloadDialog();
   connect(this,SIGNAL(SignalSetSongValue(QString)), d, SLOT(SlotSetNameValue(QString)));
   connect(d,SIGNAL(rejected()),this,SLOT(delete_downloadDialog()));
   (*d).show();
   emit SignalSetSongValue(song_trans);

}
void MyMainDlg::delete_downloadDialog()
{
    d->deleteLater();
}
void MyMainDlg::slotmynetwork(QByteArray byt)//由mynetwork发的信号槽
{
   ...
   qDebug()<<obj1.value("songs").isArray();
   ...
   for(int i=0;i<arrycount;i++)//在这里完成搜索插入列表
   {
      ...
      searchtable->insertRow(count);
      searchtable->setItem(count,0,new QTableWidgetItem(songname));
      searchtable->setItem(count,1,new QTableWidgetItem(author));
      searchtable->setItem(count,2,new QTableWidgetItem(""));
      searchtable->setCellWidget(count,2,wid);

      if(i==arrycount)//出现Bug点击最后一行
      {
          searchtable->insertRow(i);
          searchtable->setItem(i,0,new QTableWidgetItem(""));
          searchtable->setItem(i,1,new QTableWidgetItem(""));
          searchtable->setItem(i,2,new QTableWidgetItem(""));
      }
       else
      {
        int count=searchtable->rowCount();
...
void MyMainDlg::setDialogStyleSheet()
{
    ...
    order2_style="QPushButton{background:transparent;}"
            "QPushButton:hover{border-image: url(:image/order1_hover);}";
    order0_style="QPushButton{background:transparent;}"
            "QPushButton:hover{border-image: url(:image/order2_hover);}";
    order1_style="QPushButton{background:transparent;}"
            "QPushButton:hover{border-image: url(:image/order3_hover);}";
    QString slider_style="QSlider::groove:horizontal{border:0px;height:4px;width:250px}"
                         "QSlider::sub-page:horizontal{background:white;}"
                         "QSlider::add-page:horizontal{background:lightgray;}"
                         "QSlider::handle:horizontal{background:white;width:15px;border-radius:5px;margin:-3px 0px -3px 0px;}";
    QString slider_style_vol="QSlider::groove:horizontal{border:0px;height:4px;width:60px}"
                         "QSlider::sub-page:horizontal{background:white;}"
                         "QSlider::add-page:horizontal{background:lightgray;}"
                         "QSlider::handle:horizontal{background:white;width:15px;border-radius:5px;margin:-3px 0px -3px 0px;}";
    QString vol_style="QPushButton{background:transparent;}"
...
//    ui->groupBox_4->hide();
    ...
    ui->pushButton_vol->setStyleSheet(vol_style);
    ui->horizontalSlider_vol->setStyleSheet(slider_style_vol);
    ...
    ui->horizontalSlider->setGeometry(240,630
                                     ,300,30);//进度条
//    ui->widget->hide();

    ui->horizontalSlider->raise();
    ui->pushButton_next->raise();
ui->pushButton_backward->raise();
ui->pushButton_playpause->raise();
    ui->listWidget_side->setStyleSheet("QListWidget{background-color:rgb(0, 0, 0,130);}"
                                       "QListWidget::item{height: 40;font-size : 30px;};");//
    //两种样式
...
void MyMainDlg::setbtnmenu()//里面有包含按钮的菜单
{
    ...
    /////////////////////////////////////////
    actionLoop =new QAction("  顺序播放",this);
...
void MyMainDlg::trayiconmenu()
{
    ...
    backward_button->setFixedSize(21,21);
    forward_button->setFixedSize(21,21);
    ...
    music_name_label->setText("极视听音乐播放器");
    ...
    trayIcon->setToolTip("欢迎使用-音乐播放器");
...
///////////////////////////////////////////////////////////////////////Execellentlrc文件夹下的
 ...
 if(file1.exists())//如果文件存在
 {
   ...
   else if(filesave.exists())//如果存在在D:ExcellentLrc中的话
      {
...
bool MyMainDlg::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if(obj==ui->horizontalSlider_vol)                          //判断是哪个控件
      {
...
void MyMainDlg::ShowContextMenu(const QPoint& pos)
{
       QPoint globalPos =  mymaintable->mapToGlobal(pos);
           ...
           QAction *addlove=new QAction("  添加到我喜欢",this);
           QAction *cancellove=new QAction("取消我喜欢",this);
           ...
           if(mylove[mymaintable->currentRow()]==0)
               myMenu->addAction(addlove);
           else
                myMenu->addAction(cancellove);
           }
           ...
           connect(opensysfile,SIGNAL(triggered()),this,SLOT(opensysfile()));
            connect(addlove,SIGNAL(triggered()),this,SLOT(setmylove()));
             connect(cancellove,SIGNAL(triggered()),this,SLOT(setmylove()));
           myMenu->exec(globalPos);
...
{
    this->showNormal();
    if(QMessageBox::Ok==QMessageBox::information(this,"关于音乐播放器",""
                                                 "实现的功能有:\n"
                                                 "1.程序列表自动保存\n"
                                                 "2.drag和drop事件，即托动文件到程序即可添加至播放列表\n"
                                                 "3.网络自动获取歌词文件,并保存至D://ExcellentLrc\n"
                                                 "4.多线程处理歌词"
                                                 ,QMessageBox::Ok))
        return;
...
void MyMainDlg::modeAction(QAction *act)//播放模式
{
    ...
    if (act == actionCurrentItemInLoop)
    {
        ...
        ui->pushButton_order->setStyleSheet(order1_style);
         ui->pushButton_order->setToolTip("循环播放");
    }
    else if (act == actionLoop)
    {
        mynetmaintable->mysetPlaybackMode(0);
        mymaintable->mysetPlaybackMode(0);//循环
         ui->pushButton_order->setStyleSheet(order0_style);
          ui->pushButton_order->setToolTip("顺序播放");
    }
    else if (act == actionRandom)
    {
        mynetmaintable->mysetPlaybackMode(2);
        mymaintable->mysetPlaybackMode(2);//随机
         ui->pushButton_order->setStyleSheet(order2_style);
          ui->pushButton_order->setToolTip("随机播放");
    }
    // 设置当前播放模式
}
...
void MyMainDlg::positionChanged(qint64 position)
{
...
////////////////////////////////////////previous先前的
                ...
                formatpre.setFont(QFont("微软雅黑", 10, QFont::Light));
                ...
                format.setFont(QFont("微软雅黑", 12, QFont::Black));
...
void MyMainDlg::timerEvent()
{
    ...
    if(!mylanguage)
        ...
        ui->label_songscount->setText("total:"+QString::number(songcount));
...
void MyMainDlg:: pushButton_listen_clicked()
{
    fl.show();
    fl.funtion1();
}
...
void MyMainDlg::on_pushButton_vol_clicked()
{
...
void MyMainDlg::on_listWidget_side_currentRowChanged(int currentRow)//用于防止虚线
{
    if(currentRow==3)
    {
       ...
       animal1->setStartValue(QRect(590,132,194,25));
       animal1->setEndValue(QRect(160,132,194,25));
...
void MyMainDlg::on_pushButton_exit_clicked()
{
    box->hide();
    this->close();
}

void MyMainDlg::on_pushButton_mini_clicked()
{
    box->hide();
...
void MyMainDlg::on_pushButton_order_clicked()
{
    if(mymaintable->mygetNowmode()==0)
    {
        ui->pushButton_order->setStyleSheet(order1_style);
        ui->pushButton_order->setToolTip("单曲循环");
       mymaintable->mysetPlaybackMode(1);
       actionLoop->setChecked(0);
       actionCurrentItemInLoop->setChecked(1);

    }
    else if(mymaintable->mygetNowmode()==1)
        {
        ui->pushButton_order->setStyleSheet(order2_style);
        ui->pushButton_order->setToolTip("随机播放");
       mymaintable->mysetPlaybackMode(2);
        actionCurrentItemInLoop->setChecked(0);
        actionRandom->setChecked(1);

    }
    else if(mymaintable->mygetNowmode()==2)
    {
        ui->pushButton_order->setStyleSheet(order0_style);
        ui->pushButton_order->setToolTip("顺序播放");
        mymaintable->mysetPlaybackMode(0);
        actionRandom->setChecked(0);
         actionLoop->setChecked(1);
...
void MyMainDlg::on_pushButton_language_clicked()
{
    update();
    if(mylanguage)
    {
        ui->checkBox->setText("歌词");
         ui->label_title->setText("音乐播放器");
          ui->pushButton_language->setText("Eng");
           //ui->label_songscount->setText("歌曲数"+QString::number(songcount));
          mylistWidgetItem[0]->setText("歌词");
          mylistWidgetItem[1]->setText("本地列表");
           mylistWidgetItem[2]->setText("网络列表");
             mylistWidgetItem[3]->setText("歌曲搜索");
              mylistWidgetItem[4]->setText("下载管理");
              mylistWidgetItem[5]->setText("我喜欢");
              ui->lineEdit_search->setPlaceholderText("搜索列表歌曲");
              ui->pushButton->setText("视频");
              ui->label_radio->setText("录音");
              ui->label_just->setText("随心");


    }
    else
    {
        ui->checkBox->setText("LRC");
       ui->label_title->setText("Music Player");
       ui->pushButton_language->setText("中");
       //ui->label_songscount->setText("total"+QString::number(songcount));
        mylistWidgetItem[0]->setText("Lyric");
        mylistWidgetItem[1]->setText("Local ");
         mylistWidgetItem[2]->setText("Web");
           mylistWidgetItem[3]->setText("Search");
            mylistWidgetItem[4]->setText("Manage");
            mylistWidgetItem[5]->setText("Love");
            ui->lineEdit_search->setPlaceholderText("search list songs");
            ui->pushButton->setText("Video");
            ui->label_radio->setText("REC");
            ui->label_just->setText("AR");
...
void MyMainDlg::on_pushButton_clicked()
{
    v.show();

//    QProcess *p = new QProcess(this);
//       QString str = "C:/Users/hp-2/Desktop/Qt5.3player/build-customvideowidget-Desktop_Qt_5_2_1_MinGW_32bit-Debug/debug/customvideowidget.exe";
//       //p->setArguments(&str);
//       p->startDetached(str);
//      // close();
...
void MyMainDlg::showrecord()
{
    w.show();
}
void MyMainDlg::setmylove()
{
    // QTableWidgetItem *item= mymaintable->currentItem();
     int i=mymaintable->currentItem()->row();
    // int j=mymaintable->currentItem()->column();
     mylove[i]=!mylove[i];
     if(mylove[i]==1)
     {
         myloveptr[i]=mymaintable->item(i,0);
         int rowcount= mylovetable->rowCount();
         mylovetable->insertRow(rowcount);
         mylovetable->setItem(rowcount,0,new QTableWidgetItem( myloveptr[i]->text()));
         mylovetable->setItem(rowcount,1,new QTableWidgetItem( mymaintable->item(i,1)->text()));
     }
     else
     {
         int record=0;

         for( int j=0;j<mylovetable->rowCount();j++)
         {
             if(mylovetable->item(j,0)->text()==mymaintable->currentItem()->text())
                {record=j;break;}

         }

         mylovetable->removeRow(record);


     }

     // mylovetable->item(rowcount,1)->setTextAlignment(Qt::AlignRight|Qt::AlignVCenter);
}
void MyMainDlg::mylovedoubleclick(QTableWidgetItem *e)
{
    int record=0;
    for(int i=0;i<mymaintable->rowCount();i++)
    {
        if(mymaintable->item(i,0)->text()==e->text())
           {record=i;break;}

    }
    mymaintable->mysetCurrentMedia(record);
    player->setMedia(mymaintable->myCurrentMedia());
    player->play();
}

//bool MyMainDlg::eventFilter(QObject*watched, QEvent *event)
//{
//    if(watched==ui->pushButton_weather)
//    {
//        if(event->type()==QEvent::MouseButtonPress)
//            weather.show();
//        else if(event->type()==QEvent::MouseButtonDblClick)
...
//    }
//}
 void MyMainDlg::on_pushButton_menu_clicked()//点击菜单时切换为主菜单窗口
